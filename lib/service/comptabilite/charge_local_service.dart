...
/// Service de gestion des charges utilisant Hive via StorageService
///
/// Respecte le pattern du projet :
/// - Utilise StorageService pour la persistance (Hive)
/// - Singleton pour une instance unique
/// - Séparation des responsabilités
class ChargeLocalService {
  // Singleton
  static final ChargeLocalService _instance = ChargeLocalService._internal();
  factory ChargeLocalService() => _instance;
  ChargeLocalService._internal();
  ...
  List<Charge> getAllCharges() {
    try {
      ...
      deboger(['[ChargeLocalService] Erreur getAllCharges: $e']);
      return [];
    }
  }
  ...
  Future<Charge> addCharge(Charge charge) async {
    try {
      ...
      deboger(['[ChargeLocalService] Charge ajoutée: $newCharge']);
      return newCharge;
    } catch (e) {
      deboger(['[ChargeLocalService] Erreur addCharge: $e']);
      rethrow;
    }
  }
  ...
  Future<void> updateCharge(Charge charge) async {
    try {
      ...
      await _saveCharges(allCharges);
      deboger(['[ChargeLocalService] Charge mise à jour: ${charge.id}']);
    } catch (e) {
      deboger(['[ChargeLocalService] Erreur updateCharge: $e']);
      rethrow;
    }
  }
  ...
  Future<void> deleteCharge(int chargeId) async {
    try {
      ...
      await _saveCharges(allCharges);
      deboger(['[ChargeLocalService] Charge supprimée: $chargeId']);
    } catch (e) {
      deboger(['[ChargeLocalService] Erreur deleteCharge: $e']);
      rethrow;
    }
  }
  ...
  Future<void> markAsPaid(int chargeId, {DateTime? datePaiement}) async {
    try {
      ...
      await _saveCharges(allCharges);
      deboger(['[ChargeLocalService] Charge marquée comme payée: $chargeId']);
    } catch (e) {
      deboger(['[ChargeLocalService] Erreur markAsPaid: $e']);
      rethrow;
    }
  }
  ...
  Future<void> clearAllCharges() async {
    ...
    deboger(['[ChargeLocalService] Toutes les charges supprimées']);
  }
  ...
  // NOTE: La génération des charges récurrentes est maintenant gérée par le serveur via CRON
  // Voir: ComptabiliteApiService pour les appels API
}
